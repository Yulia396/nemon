...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    try {
        $stmt = $pdo->query("SELECT * FROM Power_Prices ORDER BY start_date ASC");
        $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
        // Si no hay datos
        if (!$data) {
            echo json_encode(["message" => "No hay precios de potencia disponibles."]);
            exit;
        }    
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $required_fields = ['power', 'start_date', 'end_date', 'username'];
    ...
    foreach (['p1', 'p2', 'p3'] as $subfield) {
        if (!isset($data['power'][$subfield]) || empty($data['power'][$subfield])) {
            echo json_encode(["message" => "Falta el campo 'power.$subfield'"]);
            exit;
        }
    }
    ...
    foreach (['p1', 'p2', 'p3'] as $price) {
        if (!is_numeric($data['power'][$price])) {
            echo json_encode(["message" => "Los precios de potencia deben ser numéricos."]);
            exit;
        }
    }
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM Power_Prices WHERE start_date <= ? AND end_date >= ? ORDER BY start_date ASC");
        ...
        if ($existing_prices) {
            foreach ($existing_prices as $existing) {
                ...
                if ($data['start_date'] <= $existing['start_date'] && $data['end_date'] >= $existing['end_date']) {
                    ...
                    $stmt = $pdo->prepare("DELETE FROM Power_Prices WHERE id = ?");
                ...
                // Caso 2: Si el nuevo intervalo solapa con el inicio del registro existente
                    ...
                    $stmt = $pdo->prepare("UPDATE Power_Prices SET start_date = ? WHERE id = ?");
                ...
                // Caso 3: Si el nuevo intervalo solapa con el final del registro existente
                    ...
                    $stmt = $pdo->prepare("UPDATE Power_Prices SET end_date = ? WHERE id = ?");
                ...
                // Caso 4: Si el nuevo intervalo solapa en medio de un registro existente - separamos en dos fragmentos
                    ...
                    $stmt = $pdo->prepare("INSERT INTO Power_Prices (p1, p2, p3, start_date, end_date, username) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            $stmt = $pdo->prepare("INSERT INTO Power_Prices (p1, p2, p3, start_date, end_date, username) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$data['power']['p1'], $data['power']['p2'], $data['power']['p3'], $data['start_date'], $data['end_date'], $data['username']]);

            echo json_encode(["message" => "Precios de potencia actualizados correctamente."]);
        } else {
            // Insertar los nuevos valores en la base de datos    
            $stmt = $pdo->prepare("INSERT INTO Power_Prices (p1, p2, p3, start_date, end_date, username) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$data['power']['p1'], $data['power']['p2'], $data['power']['p3'], $data['start_date'], $data['end_date'], $data['username']]);
            echo json_encode(["message" => "Precios de potencia insertados correctamente."]);
        } 
    } catch (Exception $e) {
